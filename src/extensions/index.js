...
Math.bounded = (v, m, M) => m <= M ? Math.min(Math.max(v, m), M) : (m + M) / 2;
Math.shift = (v, m, M) => {
    let range = M - m;
    while (v < m) v += range;
    return v % range;
...
window.domNS = function domNS(ns, name, attributes, ...children) {
    let el = ns ? document.createElementNS(ns, name) : document.createElement(name);
    Object.entries(attributes).forEach(([key, val]) =>
        dom.eventHandlers[key] ? el.addEventListener(key, val) :
        ns ? el.setAttribute(key, val) :
        el[dom.nameMap[key] || key] = val);
...
window.dom = function dom() {
    ...
    args.unshift(null);
    return domNS.apply(this, args);
}
window.appendChildren = function appendChildren(el, children) {
